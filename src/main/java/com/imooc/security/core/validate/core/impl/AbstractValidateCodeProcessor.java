...
public abstract class AbstractValidateCodeProcessor<C extends ValidateCode> implements ValidateCodeProcessor {
	...
	private String getProcessorType(ServletWebRequest request) {
		return StringUtils.substringAfter(request.getRequest().getRequestURL().toString(), "/code/");
	}
	
	private String getSessionKey(ServletWebRequest request) {
		return SESSION_KEY_PROFIEX + getValidateCodeType(request).toString().toUpperCase();
	}
	...
	public void validate(ServletWebRequest request) {
		...
		if (StringUtils.isBlank(codeInRequest)) {
			throw new ValidateCodeException(processorType + "验证码的值不为空");
		}
		...
		if (codeInSession.isExpired()) {
			...
			throw new ValidateCodeException(processorType + "验证码过期");
		}
